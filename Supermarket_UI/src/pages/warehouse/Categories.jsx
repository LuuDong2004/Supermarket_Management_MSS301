...
import { categoryService, productService, toList, withFallback } from '../../services/index.js'
...
export default function Categories() {
  ...
  const load = async () => { setLoading(true); const [categoryResult, productResult] = await Promise.all([withFallback(() => categoryService.list()), withFallback(() => productService.list())]); setCategories(toList(categoryResult.data)); setProducts(toList(productResult.data)); setSource(categoryResult.source); setLoading(false) }
  useEffect(() => { load() }, [])
  ...
  const save = async () => {
    ...
    const payload = { name: form.name.trim(), description: form.description.trim() || null, active: form.status === 'ACTIVE', requiresExpiry: form.requiresExpiry, taxGroup: form.taxGroup }
    setSaving(true)
    try { const saved = selected ? await categoryService.update(selected.id, payload) : await categoryService.create(payload); setCategories((current) => selected ? current.map((row) => row.id === selected.id ? saved : row) : [saved, ...current]); resetForm(); toast.success(selected ? 'Category updated.' : 'Category created.') } catch (error) { toast.error(error.message) } finally { setSaving(false) }
  }
  const deactivate = async () => { if (!selected) return toast.error('Select a category first.'); const accepted = await confirm({ title: 'Deactivate category?', message: `${selected.name} will remain available for historical products but cannot be selected for new products.`, confirmLabel: 'Deactivate', danger: true }); if (!accepted) return; const payload = { name: selected.name, description: selected.description || null, active: false, requiresExpiry: selected.requiresExpiry ?? false, taxGroup: selected.taxGroup || 'VAT 8%' }; try { const saved = await categoryService.update(selected.id, payload); setCategories((current) => current.map((row) => row.id === selected.id ? { ...row, ...saved } : row)); resetForm(); toast.success('Category deactivated.') } catch (error) { toast.error(error.message) } }
  return <div>
