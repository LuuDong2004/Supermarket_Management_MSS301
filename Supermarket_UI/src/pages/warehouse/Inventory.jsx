  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [inventoryResult, productResult, adjustmentResult] = await Promise.all([
        withFallback(() => inventoryService.list(), mockInventory), withFallback(() => productService.list(), mockProducts), withFallback(() => stockAdjustmentService.list(), mockStockAdjustments),
      ])
