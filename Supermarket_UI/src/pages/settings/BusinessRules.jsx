...
import {
  ScrollText, ShieldCheck, History, ClipboardList, Pencil,
} from 'lucide-react'
...
export default function BusinessRules() {
  ...
  const [enabled, setEnabled] = useState(() => Object.fromEntries(db.businessPolicies.map((p) => [p.id, true])))
  const [editing, setEditing] = useState(null)
  const [draftValue, setDraftValue] = useState('')
  const [levelFilter, setLevelFilter] = useState('all')

  const recentChange = useMemo(
    () => db.businessPolicies.reduce((latest, p) => (p.updated > latest ? p.updated : latest), '0000-00-00'),
    [],
  )
  ...
  const saveEdit = () => {
    toast.success(`Đã cập nhật quy tắc "${editing.name}".`)
    setEditing(null)
  }
  const toggleRule = (id) => {
    setEnabled((s) => {
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <StatCard label="Tổng quy tắc" value={formatNumber(db.businessPolicies.length)} icon={ClipboardList} tone="brand" hint="đang áp dụng" />
          ...
          <CardBody className="p-0">
            <DataTable
              ...
              columns={[
                ...
                { key: 'updated', header: 'Cập nhật', render: (r) => formatDate(r.updated) },
                {
                  key: 'enabled',
                  header: 'Trạng thái',
                  align: 'center',
                ...
                },
                {
                  ...
                  header: '',
                  align: 'right',
                  render: (r) => <Button variant="ghost" size="sm" icon={Pencil} onClick={() => openEdit(r)}>Sửa</Button>,
                },
              ]}
            />
          </CardBody>
