...
--
-- Order matters: MySQL validates a CHECK constraint against existing rows the
-- moment it is added. The data must be migrated off ROLE_SUPPLIER *before* the
-- new constraint (which no longer permits ROLE_SUPPLIER) is created, otherwise
-- ADD CONSTRAINT fails with "Check constraint 'chk_users_role' is violated".

-- 1. Drop the old CHECK so the data migration below is unconstrained.
ALTER TABLE users DROP CHECK chk_users_role;

-- 2. Migrate any existing supplier-role logins to the new role.
UPDATE users
...
-- 4. Every row now complies — add the new CHECK constraint with the new set.
...
-- 5. Fix the phantom ROLE_WAREHOUSE label seeded into the employees table (V4).
UPDATE employees
