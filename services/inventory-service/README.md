...
Placeholder for the future **inventory-service**. It is intentionally **not yet a
...
1. Scaffold the standard structure (copy `docs/architecture/SERVICE_TEMPLATE.md`):
   ...
   ├── src/main/java/com/mss301/inventory/...
   ├── src/main/resources/{application.yml, db/migration/}
   ├── Dockerfile
   └── pom.xml   (parent = ../../pom.xml)
   ```
2. Register it in the root `pom.xml` `<modules>` as `services/inventory-service`.
